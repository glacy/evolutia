...
def extract_math_expressions(content: str) -> List[str]:
    ...
    for match in COMBINED_MATH_PATTERN.finditer(content):
        expr = (
            match.group("block_content")
            or match.group("display_dollar")
            or match.group("display_bracket")
...
def count_math_operations(expression: str) -> Dict[str, int]:
    ...
    """
    # Optimized: Using str.count() is ~3x faster than regex findall for simple literals
    operations = {
        "integrals": expression.count("\\int") + expression.count("\\oint"),
        "derivatives": expression.count("\\partial")
        + expression.count("\\nabla")
        + expression.count("\\frac{d"),
        "sums": expression.count("\\sum") + expression.count("\\prod"),
        "vectors": expression.count("\\vec") + expression.count("\\mathbf"),
        "matrices": expression.count("\\begin{matrix}")
        + expression.count("\\begin{pmatrix}")
        + expression.count("\\begin{bmatrix}"),
        "functions": expression.count("\\sin")
