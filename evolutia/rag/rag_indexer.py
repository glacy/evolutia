...
class RAGIndexer:
    ...
    def index_materials(self, materials: List[Dict], analyzer) -> Dict[str, int]:
        ...
        for material in materials:
            # ⚡ O(N) pre-computed lookup dictionary to avoid O(N*M) nested loop
            sol_dict = {}
            for sol in material.get("solutions", []):
                if sol["exercise_label"] not in sol_dict:
                    sol_dict[sol["exercise_label"]] = sol

            # Indexar ejercicios
            exercises = material.get("exercises", [])
            for exercise_data in exercises:
                # Buscar solución correspondiente
                solution = sol_dict.get(exercise_data["label"])
