...
class RAGIndexer:
    ...
    def index_materials(self, materials: List[Dict], analyzer) -> Dict[str, int]:
        ...
        for material in materials:
            ...
            # Build lookup dictionary for solutions: exercise_label -> solution
            solutions_by_ex_label = {}
            for sol in material.get("solutions", []):
                if sol["exercise_label"] not in solutions_by_ex_label:
                    solutions_by_ex_label[sol["exercise_label"]] = sol

            for exercise_data in exercises:
                # Buscar solución correspondiente via O(1) lookup
                solution = solutions_by_ex_label.get(exercise_data["label"])
