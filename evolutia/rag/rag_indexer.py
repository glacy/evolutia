...
class RAGIndexer:
    ...
    def index_materials(self, materials: List[Dict], analyzer) -> Dict[str, int]:
        ...
        for material in materials:
            ...
            # Pre-computar diccionario de soluciones (O(N)) para evitar búsqueda anidada O(N*M)
            solutions_by_label = {}
            ...
            for exercise_data in exercises:
                # Búsqueda O(1) de la solución
                solution = solutions_by_label.get(exercise_data["label"])

                exercise = {
                    "label": exercise_data["label"],
                    "content": exercise_data.get(
                        "resolved_content", exercise_data.get("content", "")
                    ),
