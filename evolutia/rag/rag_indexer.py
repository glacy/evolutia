...
class RAGIndexer:
    ...
    def index_materials(self, materials: List[Dict], analyzer) -> Dict[str, int]:
        ...
        for material in materials:
            # Precomputar diccionario de soluciones (O(N)) preservando el primer match
            solution_dict = {}
            for sol in material.get("solutions", []):
                if sol["exercise_label"] not in solution_dict:
                    solution_dict[sol["exercise_label"]] = sol

            # Indexar ejercicios
            exercises = material.get("exercises", [])
            for exercise_data in exercises:
                # Buscar solución correspondiente en O(1)
                solution = solution_dict.get(exercise_data["label"])
