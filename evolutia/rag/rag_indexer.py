...
class RAGIndexer:
    ...
    def index_materials(self, materials: List[Dict], analyzer) -> Dict[str, int]:
        ...
        for material in materials:
            ...
            # Pre-calcular lookup de soluciones para O(1) en vez de O(N*M)
            solutions_lookup = {}
            for sol in material.get("solutions", []):
                if sol["exercise_label"] not in solutions_lookup:
                    solutions_lookup[sol["exercise_label"]] = sol

            for exercise_data in exercises:
                # Buscar solución correspondiente en el diccionario
                solution = solutions_lookup.get(exercise_data["label"])
