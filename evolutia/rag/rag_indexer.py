...
class RAGIndexer:
    ...
    def index_materials(self, materials: List[Dict], analyzer) -> Dict[str, int]:
        ...
        for material in materials:
            # Build an O(N) lookup dictionary preserving the first-match behavior
            solution_lookup = {}
            for sol in material.get("solutions", []):
                sol_label = sol.get("exercise_label")
                if sol_label and sol_label not in solution_lookup:
                    solution_lookup[sol_label] = sol

            # Indexar ejercicios
            exercises = material.get("exercises", [])
            for exercise_data in exercises:
                # Buscar solución correspondiente en O(1)
                solution = solution_lookup.get(exercise_data.get("label"))
