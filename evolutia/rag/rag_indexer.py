...
class RAGIndexer:
    ...
    def _ensure_embeddings_initialized(self):
        ...
        if provider == "openai":
            ...
            logger.info(
                f"[RAGIndexer] Inicializados embeddings de OpenAI: {model_name}"
            )
    ...
    def index_exercise(
        ...
        logger.info(
            f"Indexado ejercicio {exercise.get('label', 'unknown')}: {len(chunks)} chunks"
        )
        return chunk_ids
    ...
    def index_reading(self, content: str, metadata: Dict) -> List[str]:
        ...
        logger.info(
            f"Indexada lectura {metadata.get('title', 'unknown')}: {len(chunks)} chunks"
        )
        return chunk_ids
    ...
    def index_materials(self, materials: List[Dict], analyzer) -> Dict[str, int]:
        ...
        """
        stats = {"exercises": 0, "readings": 0, "chunks": 0}

        for material in materials:
            # Indexar ejercicios
            # Pre-compute solution lookup for O(1) matching instead of O(M) nested loop
            solutions_map = {}
            for sol in material.get("solutions", []):
                label = sol.get("exercise_label")
                # Preserve first-match behavior of the original break statement
                if label and label not in solutions_map:
                    solutions_map[label] = sol

            exercises = material.get("exercises", [])
            for exercise_data in exercises:
                # Buscar solución correspondiente (O(1) lookup)
                solution = solutions_map.get(exercise_data["label"])

                exercise = {
                    "label": exercise_data["label"],
                    "content": exercise_data.get("resolved_content", ""),
                    "source_file": material.get("file_path"),
                    "solution": solution.get("resolved_content") if solution else None,
                }
            ...
            if ("lectura" in filename.lower() or "teoria" in filename.lower()) and len(
                content_body
            ) > 200:
                metadata = {
