...
class MaterialExtractor:
    ...
    def get_all_exercises(self, materials: List[Dict]) -> List[Dict]:
        ...
        for material in materials:
            # Precompute solution lookup dict to avoid O(N*M) search
            solutions_by_ex_label = {}
            for sol in material.get('solutions', []):
                # Only keep the first match to preserve original behavior
            ...
            for exercise in material.get('exercises', []):
                # Lookup solución correspondiente
                solution = solutions_by_ex_label.get(exercise['label'])
