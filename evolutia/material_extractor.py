...
class MaterialExtractor:
    ...
    def get_all_exercises(self, materials: List[Dict]) -> List[Dict]:
        ...
        for material in materials:
            ...
            # Reemplazar búsqueda O(N*M) con un diccionario O(N) para búsquedas de soluciones O(1).
            # Se usa validación explícita para mantener la semántica de primer match del 'break' original.
            solution_map = {}
            for sol in material.get('solutions', []):
                if sol.get('exercise_label') not in solution_map:
                    solution_map[sol.get('exercise_label')] = sol
